...
fixa ou ``SegmentTimeline``) e ``SegmentList``.
"""
...
def _addressing_elements(
    ...
    for index, node in enumerate(nodes):
        for kind in ("SegmentTemplate", "SegmentList"):
            if _first_child(node, kind) is not None and index >= selected_index:
                selected_kind = kind
                selected_index = index
    if selected_kind is None:
        raise ValueError("representação sem SegmentTemplate ou SegmentList")
    ...
    *,
    ...
    for representation in representations:
        for segment in representation.segments:
            ...
            if segment.path.stat().st_size <= 0:
            ...
            rows.append(
                {
                    ...
                    "psnr_y_db": "",
                    "source_file": _logical_path(segment.path, mpd.parent),
    ...
    for representation in representations:
        total_bytes = sum(item.path.stat().st_size for item in representation.segments)
